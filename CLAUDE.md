...
- **GitHub Actions is the primary CI/CD.** [`.woodpecker/ci.yml`](.woodpecker/ci.yml) and [`.woodpecker/release.yml`](.woodpecker/release.yml) mirror the GHA workflows as a **manual-only** fallback (run via the Woodpecker UI). They never auto-trigger, so they will not create duplicate tags or releases. The Woodpecker release pipeline needs two secrets configured in Woodpecker: `release_token` (GitHub PAT with `repo` scope) and `github_repo` (owner/name, e.g. `user/ClearCache`). Optionally pass `BUMP_OVERRIDE=major|minor|patch` as a pipeline variable to force a bump.
